    ...
    if (printContent) {
      ...
      if (printWindow) {
        printWindow.document.write(`
          ...
          <html>
            <head>
              <title>Financial Reports Summary - Al Raya Trading</title>
              <style>
            ...
            <div className="bg-white p-6 rounded-lg max-w-4xl w-full max-h-[80vh] overflow-y-auto">
              ...
              <div id="print-content" className="p-4 bg-white text-black">
                <div className="header">
                  ...
                  <h2>Financial Reports Summary - {timeRange.toUpperCase()} Report</h2>
                  ...
                  <>
                    ...
                    <div className="summary-grid">
                      <div className="summary-item">
                        <div className="summary-number">{summary.totalUsers}</div>
                        <div>Total Users</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{summary.totalContainers}</div>
                        <div>Total Containers</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{formatUSD(summary.totalUSACostUSD)}</div>
                        <div>Total USA Cost (USD)</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{formatAED(summary.totalUSACostUSD * USD_TO_AED_RATE)}</div>
                        <div>Total USA Cost (AED)</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{formatAED(summary.totalUAESalesAED)}</div>
                        <div>Total UAE Sales (AED)</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{formatAED(summary.totalUAEExpensesAED)}</div>
                        <div>Total UAE Expenses (AED)</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">{formatAED(summary.totalNetProfitAED)}</div>
                        <div>Net Profit (AED)</div>
                      </div>
                      <div className="summary-item">
                        <div className="summary-number">
                          <span className={summary.totalNetProfitAED >= 0 ? "positive" : "negative"}>
                            {formatAED(summary.totalNetProfitAED)}
                          </span>
                        </div>
                        <div>Final Profit/Loss</div>
                      </div>
                    </div>
                  </>
                )}
                ...
                {userReports.length > 0 && (
                  <>
                    <div className="section-title">User Performance Summary</div>
                    <table className="data-table">
                      ...
                      <tbody>
                        {userReports.map((user) => (
                          <tr key={user.userId}>
                            <td>{user.userName}</td>
                            <td>{user.totalContainers}</td>
                            <td>{formatUSD(user.totalUSACostUSD)}</td>
                            <td>{formatAED(user.totalUSACostUSD * USD_TO_AED_RATE)}</td>
                            <td>{formatAED(user.totalUAESalesAED)}</td>
                            <td>{formatAED(user.totalUAEExpensesAED)}</td>
                            <td className={user.totalProfitAED >= 0 ? "positive" : "negative"}>
                              {formatAED(user.totalProfitAED)}
                            </td>
                          </tr>
                        ))}
                ...
                {monthlyReports.length > 0 && (
                  <>
                    <div className="section-title">Monthly Performance (AED)</div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th>Month</th>
                      ...
                      <tbody>
                        {monthlyReports.map((month) => (
                          <tr key={month.month}>
                            <td>{month.month}</td>
                            <td>{formatAED(month.salesAED)}</td>
                            <td>{formatAED(month.expensesAED)}</td>
                            <td className={month.profitAED >= 0 ? "positive" : "negative"}>
                              {formatAED(month.profitAED)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="section-title">Exchange Rate & Final Summary</div>
                <table className="data-table">
                  <tbody>
                    <tr>
                    ...
                    <tr>
                      <td><strong>Total Investment (USD to AED):</strong></td>
                      <td>{formatAED(summary?.totalUSACostUSD * USD_TO_AED_RATE || 0)}</td>
                    </tr>
                    <tr>
                      <td><strong>Total Revenue (AED):</strong></td>
                      <td>{formatAED(summary?.totalUAESalesAED || 0)}</td>
                    </tr>
                    <tr>
                      <td><strong>Total Expenses (AED):</strong></td>
                      <td>{formatAED((summary?.totalUSACostUSD * USD_TO_AED_RATE || 0) + (summary?.totalUAEExpensesAED || 0))}</td>
                    </tr>
                    <tr className={summary?.totalNetProfitAED >= 0 ? "positive" : "negative"}>
                      <td><strong>Net Profit/Loss (AED):</strong></td>
                      <td><strong>{formatAED(summary?.totalNetProfitAED || 0)}</strong></td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div className="footer">
                  ...
                  <p>Exchange Rate: 1 USD = {USD_TO_AED_RATE} AED</p>
                </div>
              </div>
            </div>
          </div>
